...
export default function App() {
  ...
  function handleAddWatched(newMovie) {
    ...
    // // local storage of data - method 1
...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  // find for id in watched list and if true return the userRating for that movie
  ...
  useEffect(
    function () {
      // countRef.current = countRef.current + 1 // OR countRef.current += 1 // OR
      if (userRating) countRef.current++;
    },
    [userRating]
  );

  // Destructing the array... also renaming them and making everything in lower case.
  const {
    Title: title,
    Year: year,
    Poster: poster,
  ...
  useEffect(
    function () {
      ...
      // A cleaup function is simply a function that we return from an effect!
      return function () {
          ...
          <section>
            <div className="rating">
              {!isWatched ? (
                ...
                <p>
                  Movie already in List. Your rating {watchedUserRating}{" "}
                  <span>⭐️</span>
                </p>
                // <p>{`Movie already in List. Your rating ${watchedUserRating} ⭐️`}</p>
              )}
            </div>
            <p>
              <em>{plot}</em>
